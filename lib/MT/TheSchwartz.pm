...
sub _has_enough_swap {
    ...
    } ## end if ( $mem_limit && $memory_module)

    # default to returning true
    # i.e., yes there is enough
    return 1;
} ## end sub _has_enough_swap
  ...
  JOB:
    while ( my $job = shift @jobs ) {
        ...
        my $handle = TheSchwartz::JobHandle->new(
                          { dsn_hashed => $hashdsn, jobid => $job->jobid, } );
        $handle->client($client);
        $job->handle($handle);
        return $job;
    } ## end while ( my $job = shift @jobs)
